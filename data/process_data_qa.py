...
class MediqaQADataset(Dataset):
    def __init__(self, data_dir, query_file, closed_qa_file, mode='train', transform=None):
        ...
        self.transform = transform or transforms.Compose([
            transforms.Resize((224, 224)),  # Phù hợp với CLIP
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.481, 0.457, 0.408], std=[0.269, 0.271, 0.282]),  # Chuẩn hóa cho CLIP
        ])
        ...
        with tqdm(total=len(self.queries), desc="Processing queries", unit="query") as pbar:
            for query in self.queries:
                ...
                # Gộp query_title và query_content
                query_content = (query.get('query_title_en', '') + " " + query.get('query_content_en', '')).lower().strip()
                if not query_content:
                    query_content = "skin issue"
                
                # Trích xuất từ khóa bằng spaCy
                doc = nlp(query_content)
                keywords = ' '.join([ent.text for ent in doc.ents if ent.label_ in ['DISEASE', 'SYMPTOM', 'ANATOMY']])
                if not keywords:
                ...
                for img_id in image_ids:
                    ...
                    # Tạo dữ liệu QA cho tất cả câu hỏi
                    for qa in self.closed_qa_dict:
                        qid = qa['qid']
